...
function addPurchaseRecord(name, courses){
    if (!name || typeof name !== 'string' || !members.includes(name)) {
      console.log('輸入錯誤，請輸入有效的會員名稱');
      return;
    }
    if(!courses || typeof courses !== 'number' || courses <=0 ){
    ...
    const totalPrice = courses * pricePerCourse
...
function filterNoPurchaseMember(){
  ...
  console.log(`未購買過課程的會員有：${noPurchaseMembers}`);
}
